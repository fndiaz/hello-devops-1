...
resource "aws_iam_role_policy_attachment" "hello-cluster-AmazonEKSServicePolicy" {
...
resource "aws_security_group_rule" "hello-cluster-ingress-node-https" {
  description              = "Allow pods to communicate with the cluster API Server"
  from_port                = 443
  protocol                 = "tcp"
  security_group_id        = "${aws_security_group.hello-cluster.id}"
  source_security_group_id = "${aws_security_group.hello-node.id}"
  to_port                  = 443
  type                     = "ingress"
}

resource "aws_security_group_rule" "hello-cluster-ingress-workstation-https" {
